...
zcat "$file".gz | tr -d '\r' | cut -d" " -f2 | sort -S10g -T. -u --parallel=30 | gzip -c > "$file".Top.gz
zcat "$file".gz | tr -d '\r' | cut -d" " -f3 | sort -S10g -T. -u --parallel=30 | gzip -c > "$file".Bot.gz

echo "sampling $N random triplets"

zcat "$file".T.gz | python3 random_sample.py "$N" > "$file".T_sampled
zcat "$file".Top.gz | python3 random_sample.py "$N" > "$file".U_sampled
zcat "$file".Bot.gz | python3 random_sample.py "$N" > "$file".V_sampled
...
cat "$file".tuv_samples | awk '{if ($2!=$3) print($0);}' | sort -S10g -T. -u --parallel=30 | sort -S10g -T. -nk1,1 --parallel=30 > "$file".tuv_samples.clean
