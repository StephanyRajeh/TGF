...
zcat "$file".gz | tr -d "\r" | awk '{if ($3<=$2){ print "PROBLEM!!! unsorted nodes in original file: $0"; exit -1;}}'
if test $? -ne 0
 then exit
fi
...
n_ok=`zcat "$file"_"$percentage"_injected.gz | cut -d" " -f1,2,3 | awk '{if ($2!=$3) print $0;}' | awk '{if ($2>$3) print $1,$3,$2; else print $0;}' | sort -T. -S10g -u --parallel=30 | wc -l`
